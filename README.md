...
## 📌 주제 및 선정배경
자동차 산업에서는 품질 관리(QA/QC)를 위해 불량 부품 이미지를 빠르게 분류하고 기록하는 것이 중요합니다. **자동차 품질 관리 보조 시스템(VisionQC)**는 GPT 기반 시각 인식 모델을 활용하여 불량 이미지를 자동 분류·설명하고, SQLite DB에 체계적으로 저장함으로써 품질 데이터의 축적과 분석을 용이하게 하는 것을 목표로 합니다.
